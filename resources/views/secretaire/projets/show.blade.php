@extends('layouts.app')
@section('title', 'Projet – ' . $project->responsable_nom)
@section('page-title', $project->assignment?->title ?? 'Projet')
@section('page-subtitle', $project->structure?->name ?? 'Détail du projet')
...
<div class="max-w-3xl mx-auto space-y-5">
        ...
        <div class="px-6 pt-6 pb-5 flex items-start gap-4">
            ...
            <div class="flex-1 min-w-0">
                ...
                <div class="flex flex-wrap gap-2 mt-3">
                    ...
                    @else
                    <span class="inline-flex items-center gap-1 text-xs font-semibold px-2.5 py-1 rounded-full bg-gray-50 text-gray-500 border border-gray-200">Non sélectionné</span>
                    @endif
    ...
    <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-0 divide-y sm:divide-y-0 sm:divide-x divide-gray-100">
            @foreach([
                ['Protection IP', $project->protection_types, $protectionLabels, 'bg-amber-50 text-amber-700'],
                ['Valorisation',  $project->valorisation_types, $valorisationLabels, 'bg-purple-50 text-purple-700'],
                ['Impact',        $project->impact_types, $impactLabels, 'bg-emerald-50 text-emerald-700'],
                ['Présentation',  $project->presentation_formats, $presentationLabels, 'bg-blue-50 text-blue-700'],
    ...
    <a href="{{ route('secretaire.projets.index') }}" class="btn-secondary text-sm inline-flex items-center gap-1.5">
        ← Retour à la liste
    </a>
</div>
@endsection
